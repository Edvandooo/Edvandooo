<!--
  ═══════════════════════════════════════════════════════════════════
  README.md · Francisco Edvando Sousa (F.E Sousa)
  Design: UI/UX premium · futuristic · glassmorphism · neon
  GitHub Profile README — 100% compatível com markdown + HTML inline
  ═══════════════════════════════════════════════════════════════════
-->

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:00D9FF,50:7C3AED,100:2563EB&height=200&section=header&text=F.E%20Sousa&fontSize=50&fontColor=F8FAFC&animation=fadeIn&fontAlignY=35" width="100%" alt="header"/>
</div>

<br>

<!-- HERO / APRESENTAÇÃO -->
<div align="center">
  <h1>
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=28&duration=3000&pause=500&color=00D9FF&center=true&vCenter=true&width=600&lines=Olá%2C+eu+sou+Francisco+Edvando;Python+Developer+em+forma%C3%A7%C3%A3o;Automação+%2B+Inteligência+Artificial;19+anos+%C2%B7+Brasil" alt="Typing SVG" />
  </h1>
</div>

<div align="center">
  <img src="https://img.shields.io/badge/STATUS-APRENDENDO%20TODOS%20OS%20DIAS-00D9FF?style=for-the-badge&logo=github&logoColor=white&labelColor=0F172A" />
  <img src="https://img.shields.io/badge/FOCUS-PYTHON%20%7C%20IA-7C3AED?style=for-the-badge&logo=python&logoColor=white&labelColor=0F172A" />
  <img src="https://img.shields.io/badge/OBJETIVO-PRIMEIRA%20VAGA%20DEV-2563EB?style=for-the-badge&logo=target&logoColor=white&labelColor=0F172A" />
</div>

<br><br>

<!-- DASHBOARD WIDGETS -->
<table align="center" width="100%">
  <tr>
    <td align="center" width="25%" style="background: #0F172A; border-radius: 16px; padding: 12px;">
      <img src="https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white" />
      <br><strong style="color: #00D9FF;">Developer</strong>
      <br><span style="color: #94A3B8;">Iniciante · Python</span>
    </td>
    <td align="center" width="25%" style="background: #0F172A; border-radius: 16px; padding: 12px;">
      <img src="https://img.shields.io/badge/-Brasil-009739?style=flat-square&logo=brazil&logoColor=white" />
      <br><strong style="color: #00D9FF;">Localização</strong>
      <br><span style="color: #94A3B8;">Brazil</span>
    </td>
    <td align="center" width="25%" style="background: #0F172A; border-radius: 16px; padding: 12px;">
      <img src="https://img.shields.io/badge/-Idade-FF6B6B?style=flat-square&logo=cake&logoColor=white" />
      <br><strong style="color: #00D9FF;">Idade</strong>
      <br><span style="color: #94A3B8;">19 anos</span>
    </td>
    <td align="center" width="25%" style="background: #0F172A; border-radius: 16px; padding: 12px;">
      <img src="https://img.shields.io/badge/-Foco-7C3AED?style=flat-square&logo=target&logoColor=white" />
      <br><strong style="color: #00D9FF;">Foco</strong>
      <br><span style="color: #94A3B8;">Automação · IA</span>
    </td>
  </tr>
</table>

<br>

<!-- SOBRE + TECH STACK -->
<table width="100%">
  <tr>
    <td width="30%" style="background: #0F172A; border-radius: 16px; padding: 20px; vertical-align: top;">
      <h3 style="color: #00D9FF; margin-top: 0;">💻 Sobre mim</h3>
      <p style="color: #CBD5E1; font-size: 0.95rem; line-height: 1.6;">
        Estudante de desenvolvimento de software, criando projetos para evoluir e transformar ideias em código. 
        Foco em <strong style="color: #00D9FF;">automação</strong>, <strong style="color: #7C3AED;">IA</strong> e ferramentas práticas.
      </p>
      <br>
      <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
        <span style="background: #020617; color: #00D9FF; padding: 4px 12px; border-radius: 40px; font-size: 0.7rem; border: 1px solid #00D9FF20;">Python</span>
        <span style="background: #020617; color: #00D9FF; padding: 4px 12px; border-radius: 40px; font-size: 0.7rem; border: 1px solid #00D9FF20;">Git</span>
        <span style="background: #020617; color: #00D9FF; padding: 4px 12px; border-radius: 40px; font-size: 0.7rem; border: 1px solid #00D9FF20;">GitHub</span>
        <span style="background: #020617; color: #7C3AED; padding: 4px 12px; border-radius: 40px; font-size: 0.7rem; border: 1px solid #7C3AED20;">Automação</span>
        <span style="background: #020617; color: #7C3AED; padding: 4px 12px; border-radius: 40px; font-size: 0.7rem; border: 1px solid #7C3AED20;">IA</span>
        <span style="background: #020617; color: #2563EB; padding: 4px 12px; border-radius: 40px; font-size: 0.7rem; border: 1px solid #2563EB20;">Godot</span>
      </div>
    </td>
    <td width="70%" style="padding-left: 20px;">
      <h3 style="color: #00D9FF; margin-top: 0;">📊 Roadmap de aprendizado</h3>
      <br>
      <table width="100%" cellspacing="8">
        <tr>
          <td width="25%" style="color: #94A3B8; font-weight: 500;">Python</td>
          <td width="75%" style="background: #020617; border-radius: 20px; height: 8px;">
            <div style="width: 75%; background: linear-gradient(90deg, #2563EB, #00D9FF); height: 8px; border-radius: 20px;"></div>
          </td>
        </tr>
        <tr>
          <td style="color: #94A3B8; font-weight: 500;">Git</td>
          <td style="background: #020617; border-radius: 20px; height: 8px;">
            <div style="width: 60%; background: linear-gradient(90deg, #2563EB, #00D9FF); height: 8px; border-radius: 20px;"></div>
          </td>
        </tr>
        <tr>
          <td style="color: #94A3B8; font-weight: 500;">Automação</td>
          <td style="background: #020617; border-radius: 20px; height: 8px;">
            <div style="width: 45%; background: linear-gradient(90deg, #7C3AED, #00D9FF); height: 8px; border-radius: 20px;"></div>
          </td>
        </tr>
        <tr>
          <td style="color: #94A3B8; font-weight: 500;">IA</td>
          <td style="background: #020617; border-radius: 20px; height: 8px;">
            <div style="width: 30%; background: linear-gradient(90deg, #7C3AED, #00D9FF); height: 8px; border-radius: 20px;"></div>
          </td>
        </tr>
        <tr>
          <td style="color: #94A3B8; font-weight: 500;">Godot</td>
          <td style="background: #020617; border-radius: 20px; height: 8px;">
            <div style="width: 50%; background: linear-gradient(90deg, #2563EB, #7C3AED); height: 8px; border-radius: 20px;"></div>
          </td>
        </tr>
      </table>
    </td>
  </tr>
</table>

<br>

<!-- LEARNING DASHBOARD -->
<h3 align="center" style="color: #00D9FF;">📖 Learning Dashboard</h3>
<br>

<table align="center" width="100%" cellspacing="8">
  <tr>
    <td align="center" width="33%" style="background: #0F172A; border-radius: 16px; padding: 16px;">
      <strong style="color: #F8FAFC;">Python</strong>
      <br>
      <div style="width: 100%; background: #020617; height: 4px; border-radius: 10px; margin: 8px 0;">
        <div style="width: 75%; background: linear-gradient(90deg, #7C3AED, #00D9FF); height: 4px; border-radius: 10px;"></div>
      </div>
      <span style="color: #7C3AED; font-weight: 600;">75%</span>
    </td>
    <td align="center" width="33%" style="background: #0F172A; border-radius: 16px; padding: 16px;">
      <strong style="color: #F8FAFC;">Git</strong>
      <br>
      <div style="width: 100%; background: #020617; height: 4px; border-radius: 10px; margin: 8px 0;">
        <div style="width: 60%; background: linear-gradient(90deg, #7C3AED, #00D9FF); height: 4px; border-radius: 10px;"></div>
      </div>
      <span style="color: #7C3AED; font-weight: 600;">60%</span>
    </td>
    <td align="center" width="33%" style="background: #0F172A; border-radius: 16px; padding: 16px;">
      <strong style="color: #F8FAFC;">Automação</strong>
      <br>
      <div style="width: 100%; background: #020617; height: 4px; border-radius: 10px; margin: 8px 0;">
        <div style="width: 45%; background: linear-gradient(90deg, #2563EB, #00D9FF); height: 4px; border-radius: 10px;"></div>
      </div>
      <span style="color: #2563EB; font-weight: 600;">45%</span>
    </td>
  </tr>
  <tr>
    <td align="center" style="background: #0F172A; border-radius: 16px; padding: 16px;">
      <strong style="color: #F8FAFC;">IA</strong>
      <br>
      <div style="width: 100%; background: #020617; height: 4px; border-radius: 10px; margin: 8px 0;">
        <div style="width: 30%; background: linear-gradient(90deg, #2563EB, #7C3AED); height: 4px; border-radius: 10px;"></div>
      </div>
      <span style="color: #2563EB; font-weight: 600;">30%</span>
    </td>
    <td align="center" style="background: #0F172A; border-radius: 16px; padding: 16px;">
      <strong style="color: #F8FAFC;">Godot</strong>
      <br>
      <div style="width: 100%; background: #020617; height: 4px; border-radius: 10px; margin: 8px 0;">
        <div style="width: 50%; background: linear-gradient(90deg, #7C3AED, #00D9FF); height: 4px; border-radius: 10px;"></div>
      </div>
      <span style="color: #7C3AED; font-weight: 600;">50%</span>
    </td>
    <td align="center" style="background: #0F172A; border-radius: 16px; padding: 16px;">
      <strong style="color: #F8FAFC;">GDScript</strong>
      <br>
      <div style="width: 100%; background: #020617; height: 4px; border-radius: 10px; margin: 8px 0;">
        <div style="width: 40%; background: linear-gradient(90deg, #2563EB, #7C3AED); height: 4px; border-radius: 10px;"></div>
      </div>
      <span style="color: #2563EB; font-weight: 600;">40%</span>
    </td>
  </tr>
</table>

<br>

<!-- IDIOMAS + METAS -->
<table width="100%">
  <tr>
    <td width="50%" style="background: #0F172A; border-radius: 16px; padding: 20px;">
      <h3 style="color: #00D9FF; margin-top: 0;">🌐 Idiomas</h3>
      <table width="100%" cellspacing="6">
        <tr>
          <td><span style="font-size: 1.4rem;">🇧🇷</span></td>
          <td><strong style="color: #F8FAFC;">Português</strong></td>
          <td align="right"><span style="background: #020617; color: #94A3B8; padding: 2px 12px; border-radius: 40px; font-size: 0.7rem;">nativo</span></td>
        </tr>
        <tr>
          <td><span style="font-size: 1.4rem;">🇯🇵</span></td>
          <td><strong style="color: #F8FAFC;">Japonês</strong></td>
          <td align="right"><span style="background: #020617; color: #94A3B8; padding: 2px 12px; border-radius: 40px; font-size: 0.7rem;">N5</span></td>
        </tr>
        <tr>
          <td><span style="font-size: 1.4rem;">🇺🇸</span></td>
          <td><strong style="color: #F8FAFC;">Inglês</strong></td>
          <td align="right"><span style="background: #020617; color: #94A3B8; padding: 2px 12px; border-radius: 40px; font-size: 0.7rem;">básico</span></td>
        </tr>
      </table>
    </td>
    <td width="50%" style="padding-left: 20px;">
      <h3 style="color: #00D9FF; margin-top: 0;">🎯 Metas</h3>
      <table width="100%" cellspacing="6">
        <tr>
          <td style="background: #020617; border-left: 2px solid #00D9FF; padding: 8px 12px; border-radius: 8px;">
            <span style="color: #64748B; font-size: 0.6rem; text-transform: uppercase;">vaga</span>
            <br><strong style="color: #F8FAFC;">primeira</strong>
          </td>
          <td style="background: #020617; border-left: 2px solid #7C3AED; padding: 8px 12px; border-radius: 8px;">
            <span style="color: #64748B; font-size: 0.6rem; text-transform: uppercase;">projetos</span>
            <br><strong style="color: #F8FAFC;">criar</strong>
          </td>
        </tr>
        <tr>
          <td style="background: #020617; border-left: 2px solid #2563EB; padding: 8px 12px; border-radius: 8px;">
            <span style="color: #64748B; font-size: 0.6rem; text-transform: uppercase;">IA</span>
            <br><strong style="color: #F8FAFC;">aprender</strong>
          </td>
          <td style="background: #020617; border-left: 2px solid #00D9FF; padding: 8px 12px; border-radius: 8px;">
            <span style="color: #64748B; font-size: 0.6rem; text-transform: uppercase;">automação</span>
            <br><strong style="color: #F8FAFC;">dominar</strong>
          </td>
        </tr>
      </table>
    </td>
  </tr>
</table>

<br>

<!-- PROJETOS -->
<h3 align="center" style="color: #00D9FF;">📌 Projetos em destaque</h3>
<br>

<table width="100%" cellspacing="8">
  <tr>
    <td width="50%" style="background: #0F172A; border-radius: 16px; padding: 20px; border: 1px solid #2563EB20;">
      <h4 style="color: #F8FAFC; margin: 0;">Calculadora Python</h4>
      <p style="color: #94A3B8; font-size: 0.85rem;">Primeiro projeto: lógica, CLI e interface simples. Marco inicial.</p>
      <span style="background: #020617; color: #00D9FF; padding: 4px 12px; border-radius: 30px; font-size: 0.65rem; border: 1px solid #00D9FF20;">Python</span>
    </td>
    <td width="50%" style="background: #0F172A; border-radius: 16px; padding: 20px; border: 1px solid #7C3AED20;">
      <h4 style="color: #F8FAFC; margin: 0;">Portfólio UI</h4>
      <p style="color: #94A3B8; font-size: 0.85rem;">Landing page futurista — este README.</p>
      <span style="background: #020617; color: #7C3AED; padding: 4px 12px; border-radius: 30px; font-size: 0.65rem; border: 1px solid #7C3AED20;">HTML · CSS</span>
    </td>
  </tr>
  <tr>
    <td colspan="2" style="background: #0F172A; border-radius: 16px; padding: 20px; border: 1px solid #00D9FF20;">
      <h4 style="color: #F8FAFC; margin: 0;">Em progresso: automação · IA · Godot</h4>
      <p style="color: #94A3B8; font-size: 0.85rem;">Próximos projetos: ferramentas de automação, agentes de IA, jogos em Godot.</p>
      <span style="background: #020617; color: #2563EB; padding: 4px 12px; border-radius: 30px; font-size: 0.65rem; border: 1px solid #2563EB20;">roadmap</span>
    </td>
  </tr>
</table>

<br>

<!-- GITHUB ANALYTICS -->
<h3 align="center" style="color: #00D9FF;">📊 GitHub Analytics</h3>
<br>

<table width="100%">
  <tr>
    <td width="65%" style="background: #0F172A; border-radius: 16px; padding: 20px;">
      <table width="100%">
        <tr>
          <td><span style="color: #94A3B8;">repos</span> <br><strong style="color: #F8FAFC;">8</strong></td>
          <td><span style="color: #94A3B8;">commits</span> <br><strong style="color: #F8FAFC;">127</strong></td>
          <td><span style="color: #94A3B8;">PRs</span> <br><strong style="color: #F8FAFC;">3</strong></td>
        </tr>
        <tr>
          <td><span style="color: #94A3B8;">issues</span> <br><strong style="color: #F8FAFC;">2</strong></td>
          <td><span style="color: #94A3B8;">streak</span> <br><strong style="color: #F8FAFC;">4d</strong></td>
          <td><span style="color: #94A3B8;">trophies</span> <br><strong style="color: #F8FAFC;">6</strong></td>
        </tr>
      </table>
    </td>
    <td width="35%" style="padding-left: 15px; vertical-align: top;">
      <div style="background: #0F172A; border-radius: 16px; padding: 20px;">
        <span style="color: #94A3B8; font-size: 0.7rem;">top lang</span>
        <br><strong style="color: #00D9FF; font-size: 1.2rem;">Python 68%</strong>
        <br><span style="color: #64748B; font-size: 0.7rem;">HTML 20% · CSS 12%</span>
        <div style="width: 100%; background: #020617; height: 4px; border-radius: 10px; margin-top: 8px;">
          <div style="width: 68%; background: linear-gradient(90deg, #2563EB, #00D9FF); height: 4px; border-radius: 10px;"></div>
        </div>
      </div>
    </td>
  </tr>
</table>

<br>

<!-- CONTRIBUIÇÃO + SNAKE -->
<table width="100%">
  <tr>
    <td width="70%" style="background: #0F172A; border-radius: 16px; padding: 16px;">
      <span style="color: #64748B; font-size: 0.6rem; letter-spacing: 0.04em;">CONTRIBUIÇÃO (SIMULADA)</span>
      <br>
      <div style="display: flex; gap: 4px; flex-wrap: wrap; margin-top: 6px;">
        <span style="background: #0F172A; padding: 2px 8px; border-radius: 4px; font-size: 0.6rem; color: #64748B;">▮</span>
        <span style="background: #0F172A; padding: 2px 8px; border-radius: 4px; font-size: 0.6rem; color: #64748B;">▮</span>
        <span style="background: #2563EB40; padding: 2px 8px; border-radius: 4px; font-size: 0.6rem; color: #2563EB;">▮</span>
        <span style="background: #00D9FF30; padding: 2px 8px; border-radius: 4px; font-size: 0.6rem; color: #00D9FF;">▮</span>
        <span style="background: #7C3AED30; padding: 2px 8px; border-radius: 4px; font-size: 0.6rem; color: #7C3AED;">▮</span>
        <span style="background: #0F172A; padding: 2px 8px; border-radius: 4px; font-size: 0.6rem; color: #64748B;">▮</span>
        <span style="background: #00D9FF30; padding: 2px 8px; border-radius: 4px; font-size: 0.6rem; color: #00D9FF;">▮</span>
        <span style="background: #0F172A; padding: 2px 8px; border-radius: 4px; font-size: 0.6rem; color: #64748B;">▮</span>
        <span style="background: #2563EB40; padding: 2px 8px; border-radius: 4px; font-size: 0.6rem; color: #2563EB;">▮</span>
        <span style="background: #0F172A; padding: 2px 8px; border-radius: 4px; font-size: 0.6rem; color: #64748B;">▮</span>
      </div>
    </td>
    <td width="30%" style="padding-left: 15px;">
      <div style="background: #0F172A; border-radius: 16px; padding: 16px; display: flex; align-items: center; gap: 10px;">
        <span style="font-size: 1.5rem;">🐍</span>
        <div>
          <span style="color: #64748B; font-size: 0.6rem; letter-spacing: 0.04em;">SNAKE</span>
          <br><span style="color: #00D9FF; font-size: 0.8rem;">em progresso</span>
        </div>
      </div>
    </td>
  </tr>
</table>

<br>

<!-- QUOTE -->
<div align="center" style="background: linear-gradient(135deg, #0F172A 0%, #020617 100%); border-radius: 32px; padding: 32px 40px; border: 1px solid #7C3AED20;">
  <blockquote style="font-size: 1.3rem; color: #E2E8F0; font-style: italic; margin: 0;">
    “Transformando ideias em código, uma linha de cada vez.”
  </blockquote>
  <p style="color: #64748B; margin-top: 12px; font-size: 0.85rem;">— Francisco Edvando Sousa</p>
</div>

<br>

<!-- FOOTER -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:00D9FF,50:7C3AED,100:2563EB&height=120&section=footer" width="100%" alt="footer"/>
</div>

<div align="center">
  <br>
  <img src="https://img.shields.io/github/followers/Edvandooo?style=social" />
  <img src="https://img.shields.io/github/stars/Edvandooo?style=social" />
  <img src="https://komarev.com/ghpvc/?username=Edvandooo&color=00D9FF&style=flat-square" alt="visitors" />
  <br><br>
  <span style="color: #64748B; font-size: 0.75rem;">✧ F.E Sousa · 2026 ✧</span>
</div>

<!-- 
  ═══════════════════════════════════════════════════════════════════
  FIM — README desenvolvido com design premium · UI/UX futurista
  ═══════════════════════════════════════════════════════════════════
-->
